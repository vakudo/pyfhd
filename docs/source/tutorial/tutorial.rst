...
.. note::
  ...
  In order to perform gridding in pyfhd, you need to import a beam. The beams that
  are currently supported are those generated by ``FHD``. Currently, ``pyfhd``
  is unable to form a beam (although work has been done on
  ...
  so it should only be done once. ``pyfhd`` will convert any beam ``sav``
...
Deconvolution
++++++++++++++
Deconvolution is not currently implemented in ``pyfhd``. With that being said, inside
